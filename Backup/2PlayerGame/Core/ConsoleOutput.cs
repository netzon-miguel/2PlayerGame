...
namespace _2PlayerGame
{
    class ConsoleOutput : IOutputService
    {
        ...
        public bool CoinTossInfo()
        {
            ...
            Console.Write(String.Format(ApplicationSettings.CoinTossInfo_Pick));

            if (coin.CoinToss((Coins)Enum.Parse(typeof(Coins), Console.ReadLine(), true)))
            {
                Console.Write(String.Format(ApplicationSettings.CoinTossInfo_First));
                return true;
            }
            else
            {
                Console.Write(String.Format(ApplicationSettings.CoinTossInfo_Second));
                return false;
            }
            Console.Write(String.Format(ApplicationSettings.CoinTossInfo_PickInfo,
                          choice));
        }
        ...
        public void CheckWinner(IPlayer player1, IPlayer player2)
        {
            ...
            {
                Console.Write(String.Format(ApplicationSettings.CoinTossInfo_P1Win));
            }
            else
            {
                Console.Write(String.Format(ApplicationSettings.CoinTossInfo_P2Win));
            }
            Console.Write(String.Format(ApplicationSettings.CoinTossInfo_Goodbye));
        }
